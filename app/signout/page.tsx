...
export default function SignOutPage() {
  ...
  useEffect(() => {
    const handleSignOut = async () => {
      try {
        ...
        toast.success('You have been signed out successfully', { position: 'top-center' });
        router.push('/signin');
      } catch (error) {
        toast.error('Failed to sign out. Please try again.', { position: 'top-center' });
        router.push('/');
      }
    };
  ...
  return (
    <div className="flex items-center justify-center min-h-screen bg-gradient-to-br from-purple-50 to-white dark:from-gray-900 dark:to-gray-800 p-4">
      <Card className={`w-full max-w-md shadow-lg border ${COLOR_THEME.primary.border} dark:border-gray-700 dark:bg-gray-900`}>
        <CardHeader className="space-y-4 text-center">
          <div className={`mx-auto flex h-16 w-16 items-center justify-center rounded-full ${COLOR_THEME.primary[100]} dark:bg-gray-800`}>
